...
// Note: idc/us-east-1 is ambiguous because it can be the auth region while the
// profile lives in a regional Amazon Q data-plane endpoint.
...
// shouldProbeFallbackRegions reports whether the configured account region is
// ambiguous enough to justify probing fallback regions. AWS IdC can authenticate
// through us-east-1 while the Q Developer profile lives in another data-plane
// region, so idc/us-east-1 is treated as ambiguous.
func shouldProbeFallbackRegions(account *config.Account) bool {
	...
	authMethod := strings.TrimSpace(account.AuthMethod)
	if strings.EqualFold(authMethod, "external_idp") {
		return true
	}
	return strings.EqualFold(authMethod, "idc") && region == "us-east-1"
}
