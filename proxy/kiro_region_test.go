...
// codewhisperer.{region} host, and us-east-1/empty are no-ops.
...
// Builder ID accounts that already carry their authoritative region are probed
...
func TestKiroProfileRegionCandidatesEnvOverride(t *testing.T) {
	...
	// An idc account with us-east-1 auth region is also ambiguous and uses the env fallbacks.
	got = kiroProfileRegionCandidates(&config.Account{AuthMethod: "idc", Region: "us-east-1"})
	assertOrder(t, got, []string{"us-east-1", "eu-west-1", "ap-south-1"})
}
