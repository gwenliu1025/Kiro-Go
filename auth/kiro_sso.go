...
const (
	...
	// redirects back to once sign-in succeeds. Azure tenant SSO uses a client app
	// whose registered redirect URI is localhost, so deployments must tunnel or
	// otherwise forward the operator browser's localhost:3128 to this listener.
...
// closeAllKiroSsoSessions tears down stale hosted-portal logins before starting
// a new one. Kiro uses a fixed loopback redirect port, so only one SSO attempt
// can own the callback listener at a time.
func closeAllKiroSsoSessions() {
	kiroSsoSessionsMu.Lock()
	sessions := make([]*KiroSsoSession, 0, len(kiroSsoSessions))
